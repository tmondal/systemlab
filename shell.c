...
int parseGreater(char *command){
	char *leftArgs[MAXARGS], *rightArgs[MAXARGS];
	int i;
	for (i = 0; i < 2; ++i)
	{
...
int main(int argc, char const *argv[]){
	...
	char commandString[COMMANDLEN];
	char *parsedArgs[MAXARGS], *rightArgs[MAXARGS];
	int flag;
	// Welcome message
	welcomeMessage();
	...
	while(1){
		...
		*/
		...
		if(!pf)
			continue;
		int v = parseCommand(commandString,parsedArgs);
		if(!v)
			continue;
		/*
